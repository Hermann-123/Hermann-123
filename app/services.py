...
class AIRiskManager:
    async def evaluate_match(self, match: MatchData, sim: SimulationResult) -> AIAuditReport:
        ...
        prompt = f"Tipster pro. Match : {match.home_team} vs {match.away_team}. Rédige une analyse en 2 phrases ou réponds VETO."
        try:
    ...
    def build_portfolio(self, evaluated_matches: List[Tuple[MatchData, SimulationResult, AIAuditReport]]):
        ...
        for match, sim, ai in evaluated_matches:
            ...
            if sim.proba_home >= sim.proba_away:
                dc_prob, dc_odds, dc_name = sim.proba_home + sim.proba_draw, 1.28, "Double Chance : 1X"
            else:
                dc_prob, dc_odds, dc_name = sim.proba_away + sim.proba_draw, 1.32, "Double Chance : X2"
            ...
            if max(sim.proba_home, sim.proba_away) >= 58.0 and best_odds >= 1.50:
                ...
                portfolio[TicketCategory.VIP].append(self._create(TicketCategory.VIP, match, title, f"DNB : {best_team}", round(best_odds * 0.85, 2), ai))
            ...
            portfolio[TicketCategory.MARKETS].append(self._create(TicketCategory.MARKETS, match, title, "Plus de 8.5 corners", 1.72, ai))
