...
class DixonColesEngine:
    ...
    def simulate(self, match: MatchData) -> SimulationResult:
        # On utilise la méthode pro : extraire la force des équipes depuis les cotes bookmakers
        ...
        def add_candidate(m_type, selection, proba, odds_key):
            ...
            if real_odds:
                ...
                if edge > 0 and proba >= 52.0 and 1.25 <= real_odds <= 2.20:
                    candidates.append({"match": match, "market": m_type, "selection": selection, "probability": proba, "odds": real_odds, "edge": edge})
    ...
    def build_portfolio(self, evaluated_matches: List[Tuple[MatchData, dict, AIAuditReport]]):
        ...
        for match, candidate, ai_res in evaluated_matches:
            if ai_res.is_approved and candidate:
                pool.append({"match": match, "type": candidate["selection"], "odds": candidate["odds"], "proba": candidate["probability"], "edge": candidate["edge"], "ai": ai_res.justification})
        ...
        for combo in itertools.combinations(pool[:15], 2):
            ...
            if 2.2 <= round(total_odds, 2) <= 3.5:
                ...
                ticket = GeneratedTicket(category=TicketCategory.ULTRA_SAFE, match_id="combo_value", sport=SportType.SOCCER, match_title="🌟 COMBINÉ VALUE BET", bet_type=bet_text.strip(), odds=round(total_odds, 2), ai_confidence=final_proba, ai_justification=ai_text.strip())
